...
const TranscriptionDisplay = ({ transcription, processedTranscription, isRecording, isProcessing, audioLevel = 0 }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col min-h-0">
      ...
      <div
        ref={scrollRef}
        className="flex-1 bg-gray-800/90 backdrop-blur-sm rounded-lg p-4 overflow-y-auto border border-gray-600/30 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800 min-h-0"
        style={{
          scrollbarWidth: 'thin',
        ...
        {displayText ? (
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className={`leading-relaxed whitespace-pre-wrap ${isLive ? 'text-gray-200' : 'text-gray-100'}`}>
                {displayText}
                {isLive && <span className="inline-block w-2 h-4 bg-gray-400 ml-1 animate-pulse"></span>}
              </p>
            </div>
          </div>
        ) : (
